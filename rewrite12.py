...
# Replace the state declarations (Prompt 13)
...
# Prompt 14 specific replacements on jsx_part

# 1. Map replacement
map_start_str = "{r.filteredDrafts.map((d) => ("
map_end_str = "              ))}"
start_idx_map = jsx_part.find(map_start_str)
end_idx_map = jsx_part.find(map_end_str, start_idx_map) + len(map_end_str)
...
jsx_part = jsx_part[:start_idx_map] + replacement_map + jsx_part[end_idx_map:]

# 2. Import confirm modal
modal_start_str = "{/* Import confirmation modal */}"
modal_end_str = "</AnimatePresence>"
start_idx_modal = jsx_part.find(modal_start_str)
end_idx_modal = jsx_part.find(modal_end_str, start_idx_modal) + len(modal_end_str)
...
replacement_modal = """{/* Import confirmation modal */}
      ...
      />"""
jsx_part = jsx_part[:start_idx_modal] + replacement_modal + jsx_part[end_idx_modal:]

# 3. Mobile "More" Drawer
drawer_start_str = "{/* Mobile \"More\" Drawer */}"
start_idx_drawer = jsx_part.find(drawer_start_str)
end_idx_drawer = jsx_part.find(modal_end_str, start_idx_drawer) + len(modal_end_str)
...
replacement_drawer = """{/* Mobile "More" Drawer */}
      ...
      />"""
jsx_part = jsx_part[:start_idx_drawer] + replacement_drawer + jsx_part[end_idx_drawer:]
