...
const STATUS_CFG: Record<string, { label: string; color: string; dot: string; bg: string }> = {
  ...
  cancelled:   { label: "Cancelled",   color: "text-[var(--tx2)] opacity-60",   dot: "dark:bg-white/20 bg-black/20",    bg: "dark:border-white/10 border-black/10 dark:bg-white/[0.03] bg-black/[0.04]" },
...
export function CalendarView({ currentRep, managerMode = false }: Props) {
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg-base)] text-[var(--tx1)] transition-colors duration-300">
      {/* ── Header ── */}
      <div className="p-6 pb-4 dark:border-b dark:border-white/[0.06] border-b border-black/[0.08] space-y-4 shrink-0">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          {/* View toggle */}
          <div className="flex items-center gap-1 p-1 dark:bg-white/5 bg-black/5 dark:border-white/10 border-black/10 rounded-xl">
            {(["day", "week"] as CalView[]).map(v => (
              <button key={v} onClick={() => setCalView(v)}
                className={cn("px-5 py-2 rounded-lg text-xs font-inter transition-all capitalize",
                  calView === v ? "bg-[var(--tx1)] text-[var(--bg-base)]" : "text-[var(--tx2)] opacity-60 hover:opacity-100 hover:text-[var(--tx1)]"
                )}>{v}</button>
            ))}
          </div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setSelectedDate(d => addDays(d, calView === "day" ? -1 : -7))}
              className="p-2 rounded-xl dark:bg-white/5 bg-black/5 dark:border-white/10 border-black/10 dark:hover:bg-white/10 hover:bg-black/10 transition-all">
            ...
            <button onClick={() => setSelectedDate(new Date())}
              className={cn("px-4 py-2 rounded-xl text-xs font-mono transition-all border",
                sameDay(selectedDate, new Date())
                  ...
                  : "dark:bg-white/5 bg-black/5 dark:border-white/10 border-black/10 dark:hover:bg-white/10 hover:bg-black/10 text-[var(--tx2)] opacity-60"
              )}>
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            {managerMode && (
              <div className="relative">
                <button onClick={() => setRepMenuOpen(o => !o)}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl dark:bg-white/5 bg-black/5 dark:border-white/10 border-black/10 text-xs text-[var(--tx2)] opacity-60 hover:opacity-100 hover:text-[var(--tx1)] transition-all">
                ...
                <AnimatePresence>
                  {repMenuOpen && (
                    <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }}
                      className="absolute right-0 mt-2 w-56 bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-2xl shadow-xl z-20 overflow-hidden py-1">
                      <button onClick={() => { setRepFilter("all"); setRepMenuOpen(false); }}
                        className={cn("w-full px-4 py-3 text-left text-xs font-mono dark:hover:bg-white/5 hover:bg-black/5 transition-all",
                          repFilter === "all" ? "text-[var(--tx1)]" : "text-[var(--tx2)] opacity-60"
                        )}>All Reps</button>
                      <button onClick={() => { setRepFilter(currentRep.id); setRepMenuOpen(false); }}
                        className={cn("w-full px-4 py-3 text-left text-xs font-mono dark:hover:bg-white/5 hover:bg-black/5 transition-all",
                          repFilter === currentRep.id ? "text-[var(--tx1)]" : "text-[var(--tx2)] opacity-60"
                      ...
                      {availableReps.filter(r => r.id !== currentRep.id).map(r => (
                        <button key={r.id} onClick={() => { setRepFilter(r.id); setRepMenuOpen(false); }}
                          className={cn("w-full px-4 py-3 text-left text-xs font-mono dark:hover:bg-white/5 hover:bg-black/5 transition-all",
                            repFilter === r.id ? "text-[var(--tx1)]" : "text-[var(--tx2)] opacity-60"
                          )}>{r.name}</button>
                      ))}
                    </motion.div>
                  )}
            ...
            <button onClick={fetchAppointments}
              className="p-2.5 rounded-xl dark:bg-white/5 bg-black/5 dark:border-white/10 border-black/10 text-[var(--tx2)] opacity-60 hover:opacity-100 hover:text-[var(--tx1)] transition-all">
        ...
        <AnimatePresence>
          {selectedAppt && (
            <motion.div
              ...
              className="w-80 shrink-0 dark:border-l dark:border-white/[0.06] border-l border-black/[0.08] flex flex-col overflow-y-auto"
            >
        ...
        {followUpPrompt && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.96, y: 12 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96 }}
              ...
              className="dark:bg-[#0d0d0d] bg-white dark:border-white/10 border-black/10 rounded-3xl p-8 w-full max-w-sm shadow-2xl text-[var(--tx1)]">
              <div className="flex items-start justify-between mb-7">
                <div>
                  <div className="flex items-center gap-2.5 mb-1">
                    {followUpPrompt.action === "no_show"
                      ? <UserX className="w-5 h-5 text-rose-400" />
                      : <XCircle className="w-5 h-5 text-[var(--tx2)] opacity-60" />}
                ...
                <button onClick={() => setFollowUpPrompt(null)} className="p-2 rounded-2xl text-[var(--tx2)] opacity-60 hover:opacity-100 hover:text-[var(--tx1)] dark:hover:bg-white/5 hover:bg-black/5 transition-all">
                  <X className="w-4 h-4" />
                </button>
              </div>
                    ...
                    <button key={opt.days} onClick={() => setFollowUpDate(d)}
                      className={cn("py-3 rounded-2xl text-sm font-medium transition-all",
                        followUpDate === d
                          ...
                          : "dark:bg-white/[0.04] bg-black/[0.04] text-[var(--tx2)] opacity-60 dark:hover:bg-white/[0.08] hover:bg-black/[0.08] hover:opacity-100 hover:text-[var(--tx2)]"
                      )}>{opt.label}</button>
                  );
                })}
              </div>
              <div className="mb-7">
                ...
                <input type="date" value={followUpDate} min={new Date().toISOString().slice(0, 10)}
                  ...
                  className="w-full dark:bg-white/[0.04] bg-black/[0.04] dark:border-white/10 border-black/10 rounded-2xl px-4 py-3 text-[var(--tx1)] text-sm focus:outline-none focus:border-rose-500/40 [color-scheme:light] dark:[color-scheme:dark]" />
              </div>
              <div className="flex gap-3">
                <button onClick={() => setFollowUpPrompt(null)}
                  className="flex-1 py-3 rounded-2xl dark:bg-white/5 bg-black/5 dark:border-white/10 border-black/10 text-[var(--tx2)] opacity-60 hover:opacity-100 hover:text-[var(--tx1)] transition-all text-sm">Back</button>
                <button onClick={confirmFollowUp}
                  className={cn("flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl transition-all text-sm font-medium",
                    followUpPrompt.action === "no_show"
                      ...
                      : "dark:bg-white/[0.07] bg-black/[0.07] dark:border-white/15 border-black/15 text-[var(--tx2)] dark:hover:bg-white/[0.12] hover:bg-black/[0.12]"
                  )}>
                  <ChevronRight className="w-4 h-4" />
      ...
      <AnimatePresence>
        {reschedModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.96, y: 12 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96 }}
              transition={{ type: "spring", damping: 28, stiffness: 320 }}
              className="dark:bg-[#0d0d0d] bg-white dark:border-white/10 border-black/10 rounded-3xl p-8 w-full max-w-md shadow-2xl text-[var(--tx1)]">
              <div className="flex items-start justify-between mb-7">
                <div>
                  <div className="flex items-center gap-2.5 mb-1">
                ...
                <button onClick={() => setReschedModal(null)} className="p-2 rounded-2xl text-[var(--tx2)] opacity-60 hover:opacity-100 hover:text-[var(--tx1)] dark:hover:bg-white/5 hover:bg-black/5 transition-all">
                  <X className="w-4 h-4" />
                </button>
              </div>
              <div className="mb-6">
                ...
                <input type="date" value={reschedDate} onChange={e => setReschedDate(e.target.value)}
                  className="w-full dark:bg-white/[0.04] bg-black/[0.04] dark:border-white/10 border-black/10 rounded-2xl px-4 py-3 text-[var(--tx1)] text-sm focus:outline-none focus:border-amber-500/50 [color-scheme:light] dark:[color-scheme:dark]" />
              </div>
              <div className="mb-6">
                  ...
                  {TIME_SLOTS.map(slot => (
                    <button key={slot} onClick={() => setReschedTime(slot)}
                      className={cn("py-2.5 rounded-xl text-xs font-medium transition-all",
                        reschedTime === slot ? "bg-amber-500 text-white" : "dark:bg-white/[0.04] bg-black/[0.04] text-[var(--tx2)] opacity-60 dark:hover:bg-white/[0.08] hover:bg-black/[0.08]"
                      )}>
                  ...
                  {DURATIONS.map(d => (
                    <button key={d.value} onClick={() => setReschedDur(d.value)}
                      className={cn("flex-1 py-2.5 rounded-xl text-xs font-medium transition-all",
                        reschedDur === d.value ? "bg-amber-500/20 border border-amber-500/40 text-amber-300" : "dark:bg-white/[0.04] bg-black/[0.04] text-[var(--tx2)] opacity-60 dark:hover:bg-white/[0.07] hover:bg-black/[0.07]"
                      )}>{d.label}</button>
                  ))}
                </div>
              </div>
              <div className="flex gap-3">
                <button onClick={() => setReschedModal(null)}
                  className="flex-1 py-3 rounded-2xl dark:bg-white/5 bg-black/5 dark:border-white/10 border-black/10 text-[var(--tx2)] opacity-60 hover:opacity-100 hover:text-[var(--tx1)] transition-all text-sm">Cancel</button>
...
function DayGrid({ appts, conflictIds, currentTimeTop, selectedId, actionLoading, onSelect, onNavigate, onReschedule, onNoShow, onCancel, onConfirm, onStartInspection }: DayGridProps) {
  ...
  return (
    <div className="flex p-6 gap-0">
      ...
      <div className="w-16 shrink-0" style={{ height: GRID_HEIGHT + HOUR_PX }}>
        {HOURS.map(h => (
          <div key={h} className="relative" style={{ height: HOUR_PX }}>
            <span className="absolute top-0 -translate-y-1/2 text-[10px] font-mono text-[var(--tx2)] opacity-40 whitespace-nowrap select-none">
              {hourLabel(h)}
            </span>
          </div>
        ))}
      ...
      {/* Grid */}
      <div className="flex-1 relative dark:border-l dark:border-white/[0.05] border-l border-black/[0.08]" style={{ height: GRID_HEIGHT }}>
        {/* Hour lines */}
        {HOURS.map(h => (
          <div key={h} className="absolute left-0 right-0 dark:border-t dark:border-white/[0.05] border-t border-black/[0.08]"
            style={{ top: (h - DAY_START_HOUR) * HOUR_PX }} />
        ))}

        {/* Half-hour lines */}
        {HOURS.slice(0, -1).map(h => (
          <div key={`half-${h}`} className="absolute left-0 right-0 dark:border-t dark:border-white/[0.025] border-t border-black/[0.04]"
        ...
        {appts.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="text-center">
              <Calendar className="w-8 h-8 text-[var(--tx2)] opacity-30 mx-auto mb-3" />
              <p className="text-[var(--tx2)] opacity-40 text-sm font-light">No appointments</p>
            </div>
          </div>
        )}
          ...
          return (
            <motion.div
              ...
              className={cn(
                ...
                isSelected ? "dark:ring-2 dark:ring-white/30 ring-2 ring-black/20 ring-offset-1 dark:ring-offset-black ring-offset-white" : "hover:ring-1 dark:hover:ring-white/20 hover:ring-black/20",
                isConflict
            ...
            >
              <div className="p-2 h-full flex flex-col overflow-hidden">
                ...
                <p className="text-[10px] font-mono text-[var(--tx2)] opacity-60 leading-tight truncate">
                  {fmtTime(appt.appointment_start_at)}
                </p>
                {height > 50 && (
                  <p className="text-xs font-inter font-medium text-[var(--tx1)] leading-tight truncate mt-0.5">
                    {address}
                  </p>
                )}
                {height > 70 && owner && (
                  <p className="text-[9px] font-mono text-[var(--tx2)] opacity-60 truncate mt-0.5">{owner}</p>
                )}
                {height > 90 && (
                  <div className="mt-auto flex items-center gap-1 pt-1 dark:border-t dark:border-white/10 border-t border-black/10">
                    {canStart && (
                    ...
                    <button onClick={e => { e.stopPropagation(); onNavigate(appt); }}
                      className="flex items-center gap-1 text-[9px] font-mono text-[var(--tx2)] opacity-60 hover:opacity-100 hover:text-[var(--tx1)] ml-auto">
        ...
        {days.map(day => {
          ...
          return (
            <div key={day.toISOString()} className={cn(
              ...
              isToday ? "border-[var(--tx2)]" : "border-[var(--border-color)]"
            )}>
              {/* Day header */}
              <button onClick={() => onSelectDay(day)}
                className={cn(
                  "w-full px-3 py-3 text-left transition-all hover:bg-[var(--bg-subtle)]",
                  isToday ? "bg-[var(--bg-subtle)]" : "bg-[var(--bg-base)]"
                )}>
                <div className="flex items-center justify-between">
                  <span className={cn("text-xs font-inter font-medium",
                    isToday ? "text-[var(--tx1)]" : "text-[var(--tx2)] opacity-60")}>{fmtDayLabel(day)}</span>
                  {hasConflict && <AlertTriangle className="w-3 h-3 text-amber-400" />}
                </div>
                {dayAppts.length > 0 && (
                  <span className="text-[9px] font-mono text-[var(--tx2)] opacity-60 mt-0.5 block">
              ...
              <div className="p-2 space-y-1 min-h-[80px] max-h-[200px] overflow-y-auto">
                {dayAppts.length === 0 ? (
                  <p className="text-[9px] font-mono text-[var(--tx2)] opacity-30 text-center py-4">—</p>
                ) : (
                  dayAppts.map(appt => {
                    const cfg = STATUS_CFG[appt.appointment_status] ?? STATUS_CFG.scheduled;
                    const isConflict = conflictIds.has(appt.id);
                    ...
                    return (
                      <button key={appt.id} onClick={() => onSelect(appt)}
                        ...
                        <p className="text-[var(--tx1)] truncate font-inter leading-tight">{address}</p>
                      </button>
                    );
                  })
                )}
    ...
    <div className="flex flex-col h-full bg-[var(--bg-elevated)]">
      ...
      <div className="p-5 border-b border-[var(--border-color)] flex items-start justify-between gap-3">
        <div className="space-y-1 min-w-0">
          ...
          <p className="text-base font-inter font-medium text-[var(--tx1)] leading-tight">{address}</p>
          {owner && <p className="text-[10px] font-mono text-[var(--tx2)] opacity-60 uppercase tracking-wider">{owner}</p>}
        </div>
        <button onClick={onClose} className="p-2 rounded-xl text-[var(--tx2)] opacity-60 hover:opacity-100 hover:text-[var(--tx1)] hover:bg-[var(--bg-subtle)] transition-all shrink-0">
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className={cn("text-[10px] font-mono uppercase tracking-wider px-3 py-1.5 rounded-lg border",
              `${cfg.color} bg-[var(--bg-subtle)] border-[var(--border-color)]`)}>{cfg.label}</span>
            {appt.pipeline_leads?.pipeline_status && (
              <span className="text-[9px] font-mono text-[var(--tx2)] opacity-60 uppercase">
                {appt.pipeline_leads.pipeline_status.replace(/_/g, " ")}
              </span>
            )}
          </div>
          <div className="flex items-center gap-2 text-sm text-[var(--tx1)]">
        ...
        {/* Contact info */}
        {(phone || address) && (
          <div className="space-y-2 p-4 bg-[var(--bg-subtle)] border border-[var(--border-color)] rounded-2xl">
            {address && (
              <div className="flex items-start gap-2">
                <MapPin className="w-3.5 h-3.5 text-[var(--tx2)] opacity-60 mt-0.5 shrink-0" />
                <span className="text-xs text-[var(--tx1)] font-light">{address}</span>
              </div>
            )}
            {phone && (
              <div className="flex items-center gap-2">
                <Phone className="w-3.5 h-3.5 text-[var(--tx2)] opacity-60 shrink-0" />
                <span className="text-xs font-mono text-[var(--tx2)] opacity-60">{phone}</span>
              </div>
            )}
          </div>
        )}
        ...
        {(appt.notes || appt.pipeline_leads?.lead_notes) && (
          <div className="p-3 bg-[var(--bg-subtle)] border-l-2 border-[var(--border-color)] rounded-r-xl">
            <div className="flex items-center gap-1.5 mb-1.5">
              <FileText className="w-3 h-3 text-[var(--tx2)] opacity-40" />
              <span className="text-[9px] font-mono text-[var(--tx2)] opacity-60 uppercase tracking-widest">Notes</span>
            </div>
            <p className="text-xs text-[var(--tx2)] opacity-60 font-light italic">
              {appt.notes || appt.pipeline_leads?.lead_notes}
            </p>
          </div>
        )}
        ...
        {appt.pipeline_leads?.next_follow_up_at && (
          <div className="flex items-center gap-2 text-xs text-[var(--tx2)] opacity-60">
            <AlarmClock className="w-3.5 h-3.5 text-[var(--tx2)] opacity-40 shrink-0" />
        ...
        <div className="grid grid-cols-2 gap-2">
          <button onClick={onCall}
            className="flex items-center justify-center gap-2 py-2.5 rounded-xl bg-[var(--bg-subtle)] border border-[var(--border-color)] text-[var(--tx2)] opacity-60 hover:opacity-100 hover:text-[var(--tx1)] hover:bg-[var(--bg-elevated)] transition-all text-xs">
      ...
      {!["cancelled", "completed"].includes(appt.appointment_status) && (
        <div className="p-5 border-t border-[var(--border-color)] flex items-center gap-4">
      ...
      {appt.appointment_status === "cancelled" && (
        <div className="p-5 border-t border-[var(--border-color)] flex items-center justify-end">
          <button onClick={onRemove} disabled={busy}
            className="flex items-center gap-1.5 text-[10px] font-mono text-[var(--tx2)] opacity-60 hover:opacity-100 hover:text-rose-400 uppercase tracking-widest transition-colors disabled:opacity-40">
